...
import com.letrogthien.wallet.repositories.ErrorLogRepository;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> handlePaymentException(CustomException ex, WebRequest request) {
        ErrorCode errorCode = ex.getErrorCode();
        ProblemDetail problemDetail = ProblemDetail.forStatusAndDetail(
                errorCode.getStatus(),
                ex.getMessage() != null ? ex.getMessage() : errorCode.getMessage());
        problemDetail.setTitle("Pay Error");
        problemDetail.setProperty("errorCode", errorCode.getCode());
        problemDetail.setProperty(TIMESTAMP, System.currentTimeMillis());
        logToMongo(request, ex, errorCode.getCode(), ex.getMessage());
        return new ResponseEntity<>(problemDetail, errorCode.getStatus());
